...
form.addEventListener('submit', function (event) {
	...
	Promise.all([_API2.default.getAirbnbByPlace(location), _API2.default.getWeatherByPlace(location).then(function (weather) {
		return _API2.default.convertKelvinToCelsius(weather.main.temp);
	})]).then(displayResults).catch(function (error) {
...
var displayResults = function displayResults(_ref) {
	var _ref2 = _slicedToArray(_ref, 2),
	    airbnbResult = _ref2[0],
	    temperature = _ref2[1].temperature;

	console.log(airbnbResult, temperature);
	loading.style.display = 'none';
	var title = new _Component2.default('h2', {}, ['Temperature : ' + temperature + '\xB0C']);
	var results = new _Component2.default('ul', {}, airbnbResult.map(function (item) {
		return (
			// pour chaque lieu on ajoute un <li> avec les infos du lieu (image + titre)
			new _Component2.default('li', { class: 'list-group-item' }, [new _Component2.default('img', { src: item.listing.picture_url, style: 'width:100%' }), new _Component2.default('h4', {}, [item.listing.name])])
		);
	}));
	var container = new _Component2.default('div', {}, [title, results]);
...
var API = function () {
	...
	_createClass(API, null, [{
		key: 'getAirbnbByPlace',

		// Appel du webservice qui permet de récupérer les logements en fonction d'un lieu
		value: function getAirbnbByPlace(location) {
			return fetch('airbnb.php?location=' + location).then(function (airbnbResult) {
				return airbnbResult.json();
			}).then(function (data) {
				return data.results_json.search_results;
			});
		}
	}, {
		key: 'getWeatherByPlace',
		value: function getWeatherByPlace(location) {
			var apiKey = '851b41b99f54374d348017676b74fd02';
			return fetch('http://api.openweathermap.org/data/2.5/weather?q=' + location + '&appid=' + apiKey, { method: 'get' }).then(function (weather) {
				return weather.json();
			});
		}
	}, {
		key: 'convertKelvinToCelsius',
		value: function convertKelvinToCelsius(temperature) {
			return fetch('temperature.php?temperature=' + temperature).then(function (temperature) {
