...
var getAirbnbByPlace = function getAirbnbByPlace(city) {
	return fetch('airbnb.php?location=' + city).then(function (airbnbResult) {
		return airbnbResult.json();
	}).then(function (data) {
		return data.results_json.search_results;
	});
};
var getWeatherByPlace = function getWeatherByPlace(city) {
	var apiKey = '851b41b99f54374d348017676b74fd02';
	return fetch('http://api.openweathermap.org/data/2.5/weather?q=' + city + '&appid=' + apiKey, { method: 'get' }).then(function (weather) {
		return weather.json();
	});
};
var convertKelvinToCelsius = function convertKelvinToCelsius(temperature) {
...
form.addEventListener('submit', function (event) {
	...
	Promise.all([getAirbnbByPlace(location), getWeatherByPlace(location).then(function (weather) {
		return convertKelvinToCelsius(weather.main.temp);
...
var displayResults = function displayResults(_ref) {
	...
	var title = ['<h2>Temperature : ' + temperature + '\xB0C</h2>'];
	var results = airbnbResult.map(function (item) {
		return (
	...
	var html = [].concat(title, _toConsumableArray(results));
